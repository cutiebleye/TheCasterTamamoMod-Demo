...
public class SpiritTheftCard extends BaseCard {
    public static final String ID = makeID("SpiritTheft"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    private static final int THEFT_AMT = 2;
    private static final int UPG_THEFT_AMT = 1;

    public SpiritTheftCard() {
        super(ID, info); //Pass the required information to the BaseCard constructor.
        setMagic(THEFT_AMT, UPG_THEFT_AMT);
    }

    public void use(AbstractPlayer p, AbstractMonster m) {
        this.addToBot(new ApplyPowerAction(p, p, new SpiritTheftPower(p, this.magicNumber), this.magicNumber));
    }

    @Override
    public AbstractCard makeCopy() { //Optional
        return new SpiritTheftCard();
    }
}
