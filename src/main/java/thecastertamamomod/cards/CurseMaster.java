...
public class CurseMaster extends ManaCard {
    public static final String ID = makeID("CurseMaster"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void useManaCard(AbstractPlayer p, AbstractMonster m) {
        this.addToBot(new ApplyPowerAction(p, p, new CurseMasterPower(p, 1)));
